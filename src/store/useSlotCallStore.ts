...
export const useSlotCallStore = create<SlotCallState>((set, get) => ({
	...
	addSlotCall: async (slotName) => {
		...
		try {
			const res = await fetch(
				"https://kingdata.onrender.com/api/slot-calls",
				{
					method: "POST",
					headers: {
	...
	submitBonusCall: async (id, slotName) => {
		...
		try {
			const res = await fetch(
				`https://kingdata.onrender.com/api/slot-calls/${id}/bonus-call`,
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
	...
	updateSlotStatus: async (id, status, x250Hit = false) => {
		...
		try {
			const res = await fetch(
				`https://kingdata.onrender.com/api/slot-calls/${id}/status`,
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
	...
	deleteSlotCall: async (id) => {
		...
		try {
			const res = await fetch(
				`https://kingdata.onrender.com/api/slot-calls/${id}`,
				{
					method: "DELETE",
					headers: {
						Authorization: `Bearer ${token}`,
	...
	fetchSlotCalls: async () => {
		...
		const url =
			userRole === "admin"
				? "https://kingdata.onrender.com/api/slot-calls"
				: "https://kingdata.onrender.com/api/slot-calls/my";
